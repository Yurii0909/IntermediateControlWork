...
public class Raffle {
        ...
        if (prizes.size() == 0) {
            ...
            for (Toy toy : toys) {
                for (int i = 0; i < toy.getToyVictoryFrequency(); i++) {
                    Toy temp = new Toy(toy.getToyId(), toy.getToyTitle(), rnd.nextInt(10) + 1);
                    prizes.add(temp);
                }
            }
        }
    ...
    private void saveResult(String text) {
        File file = new File(
                "C:\\Users\\ACER\\OneDrive\\Рабочий стол\\project\\HomeWork\\src\\main\\java\\org\\example\\Result.txt");
        try {
            file.createNewFile();
